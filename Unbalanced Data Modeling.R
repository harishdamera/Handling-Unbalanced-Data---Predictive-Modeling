...
roc=roc.curve(test$responded,rf_test_predictions)

## ROC for AUC
library(ROCR)

##scoring train data
train$predictedresponded=predict(rf_model_tomek_smote,train,type="response") #use type="prob to get probabilities
train$respondednew = ifelse(train$responded == "no",0,1 )
train$predictedrespondednew = ifelse(train$predictedresponded == "no",0,1 )

predtrain=prediction(train$predictedrespondednew,train$respondednew)
perftrain=performance(predtrain,"tpr","fpr")

##scoring test data
test$respondednew = ifelse(test$responded == "no",0,1 )
test$predictedresponded=predict(rf_model_tomek_smote,test,type="response")
test$predictedrespondednew=ifelse(test$predictedresponded=="no",0,1)

predtest=prediction(test$predictedrespondednew,test$respondednew)
...
lifttest=performance(predtest,"lift","rpp")
#plotting lift chart
plot(lifttrain,col="green",lty=1,xlab="% emailed",add=FALSE,main="Lift Chart")
par(new=TRUE)
plot(lifttest,col="red",lty=2,xlab="% emailed",add=FALSE)
...
## Smoted Data
set.seed(1234)
SVM_model_tomek_smote = svm(responded~.,train_tomek_smote,cost=20,gamma=10)
SVM_test_predictions = predict(SVM_model_tomek_smote, test, type="class")
conf_matrix_SVM = table(test$responded,SVM_test_predictions)
